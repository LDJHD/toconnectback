...
export default class ArticlesController {
    async index({ request, response }) {
        try {
            ...
            if (sousCategoryId)
                query = query.where('sousCategoryId', sousCategoryId);
            if (prixMin)
                query = query.where('prix', '>=', prixMin);
            if (prixMax)
    ...
    async allAdmin({ request, response }) {
        try {
            const page = request.input('page', 1);
            const limit = request.input('limit', 20);
            const search = request.input('search');
    ...
    async byCategory({ params, request, response }) {
        try {
            const page = request.input('page', 1);
            const limit = request.input('limit', 20);
            const sousCategoryId = request.input('sous_category_id');
    ...
    async update({ params, request, response }) {
        try {
            ...
            if (data.prixPromo !== undefined)
                article.prixPromo = data.prixPromo || null;
            if (data.categoryId !== undefined)
                article.categoryId = data.categoryId;
            if (data.sousCategoryId !== undefined)
                article.sousCategoryId = data.sousCategoryId || null;
            if (data.stock !== undefined)
                article.stock = data.stock;
            if (data.actif !== undefined)
                article.actif = data.actif;
            if (data.enVedette !== undefined)
                article.enVedette = data.enVedette;
            const images = request.files('images', { size: '5mb', extnames: ['jpg', 'jpeg', 'png', 'webp'] });
            if (images.length > 0) {
