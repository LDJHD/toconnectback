...
export default class ComptesController {
  ...
  async store({ request, response }: HttpContext) {
    try {
      ...
      const compte = await Compte.create({
        ...data,
        dateExpiration: DateTime.fromJSDate(new Date(data.dateExpiration)),  // Conversion correcte de la date
      })
  ...
  async update({ params, request, response }: HttpContext) {
    try {
      ...
      if (data.emailCompte) compte.emailCompte = data.emailCompte
      if (data.motDePasse) compte.motDePasse = data.motDePasse
      if (data.plateforme) compte.plateforme = data.plateforme
      if (data.nbUtilisateurs) compte.nbUtilisateurs = data.nbUtilisateurs
      
      // Si dateExpiration est définie, la convertir en DateTime
