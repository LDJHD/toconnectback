...
// Fonction pour gérer les notifications préventives (1 semaine avant)
async function checkPreventiveNotifications() {
  try {
    ...
    // Trouver les abonnements qui expirent dans une semaine
    ...
    for (const abonnement of abonnementsAExpirer) {
      // Vérifier si une notification préventive a déjà été envoyée pour cet abonnement
      ...
      await db.transaction(async (trx: TransactionClientContract) => {
        // Créer une notification préventive
        ...
        // Créer la notification dans le système avec l'abonnement_id
...
export async function checkExpiredAbonnements() {
  try {
    ...
    // Trouver tous les abonnements qui expirent aujourd'hui ou qui sont déjà expirés
    ...
    for (const abonnement of abonnementsExpires) {
      ...
      await db.transaction(async (trx: TransactionClientContract) => {
        // Récupérer le type de compte pour obtenir le nombre d'écrans
        ...
        await sendSms(
          abonnement.utilisateur.telephone,
          abonnement.compte,
          abonnement.profil
        )
    ...
    // Vérifier et envoyer les notifications préventives
